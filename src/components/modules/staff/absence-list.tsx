...
export function AbsenceList({ staffId }: AbsenceListProps) {
  const [open, setOpen] = useState(false);
  const [editing, setEditing] = useState<AbsenceRecord | null>(null);

  const { data: absences = [], refetch } = trpc.staff.absence.getByStaff.useQuery({ staffMemberId: staffId });
  const { data: summary } = trpc.staff.absence.getSummary.useQuery({ staffMemberId: staffId });
